...
extern int sts_check(PCCC *con, const BUF *msg)
{
  ...
  switch (sts)
    ...
    case 0x04:
      ...
      break;
    case 0x05:
      strncpy(str, "Application layer timed out waiting for response",
	      PCCC_ERR_LEN);
      break;
    case 0x06:
    ...
    case 0x10:
      ...
      break;
    case 0x20:
      strncpy(str, "Host has a problem and will not communicate",
	      PCCC_ERR_LEN);
      remote = 1;
      break;
    case 0x30:
      ...
      break;
    case 0x40:
      strncpy(str, "Host could not complete function due to hardware fault",
	      PCCC_ERR_LEN);
      remote = 1;
      break;
    case 0x50:
      strncpy(str, "Addressing problem or memory protect rungs",
	      PCCC_ERR_LEN);
      remote = 1;
      break;
    case 0x60:
      strncpy(str, "Function not allowed due to command protection selection",
	      PCCC_ERR_LEN);
      remote = 1;
      break;
    case 0x70:
  ...
  snprintf(con_priv->errstr, PCCC_ERR_LEN, "%s node %u(dec) error : %s",
	   remote ? "Remote" : "Local", msg_get_src(msg), str);
  return 1;
}
...
static void ext_sts(const BUF *msg, char *str)
{
  ...
  switch (cmd)
    ...
    default: /* Other commands shouldn't return EXT STS values. */
      snprintf(str, PCCC_ERR_LEN, "CMD 0x%x returned unexpected EXT STS 0x%x",
	       cmd, es);
      break;
    }
  return;
...
static void ext_sts_dh(const BUF *msg, char *str, uint8_t es)
{
  switch (es)
    ...
    case 0x02:
      ...
      break;
    case 0x03:
      strncpy(str, "More levels specified in address than system supports",
	      PCCC_ERR_LEN);
      break;
    case 0x04:
    ...
    case 0x07:
      ...
      break;
    case 0x08:
      strncpy(str, "Cannot complete request, situation has changed since start"
	       " of the command", PCCC_ERR_LEN);
      break;
    case 0x09:
      strncpy(str, "Data or file is too large", PCCC_ERR_LEN);
      break;
    case 0x0a:
      strncpy(str, "Transaction size plus word address is too large",
	      PCCC_ERR_LEN);
      break;
    case 0x0b:
      ...
      break;
    case 0x0c:
      strncpy(str, "Condition cannot be generated, resource is not available",
	      PCCC_ERR_LEN);
      break;
    case 0x0d:
      strncpy(str, "Condition already exists, resource is already available",
	      PCCC_ERR_LEN);
      break;
    case 0x0e:
    ...
    case 0x13:
      ...
      break;
    case 0x14:
      strncpy(str, "Command execution failure for unknown reason",
	      PCCC_ERR_LEN);
      break;
    case 0x15:
      ...
      break;
    case 0x16:
      strncpy(str, "Scanner not able to communicate with 1771 rack adapter",
	      PCCC_ERR_LEN);
      break;
    case 0x17:
...
static void ext_sts_485(const BUF *msg, char *str, uint8_t es, uint8_t cmd)
{
  switch (es)
    ...
    case 0x1b:
      ...
      break;
    default:
      snprintf(str, PCCC_ERR_LEN, "Undefined EXT STS 0x%x for CMD 0x%x", es,
	       cmd);
      break;
    }
  return;
...
static void ext_sts_1a(const BUF *msg, char *str)
{
  ...
  if (msg_get_owner_node(msg, &on))
    snprintf(str, PCCC_ERR_LEN, "File is open; node %u owns it."
	     " For SLC 5/05 node 256 indicates the Ethernet port", on);
  else strncpy(str, "File is open; another node owns it", PCCC_ERR_LEN);
  return;
}
...
static void ext_sts_1b(const BUF *msg, char *str)
{
  uint8_t on; /* Owner node */
  if (msg_get_owner_node(msg, &on))
    snprintf(str, PCCC_ERR_LEN, "Node %u is the program owner."
	     " For SLC 5/05 node 256 indicates the Ethernet port", on);
  else strncpy(str, "Another node is the program owner", PCCC_ERR_LEN);
  return;
}
