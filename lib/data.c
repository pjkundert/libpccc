...
extern PCCC_RET_T data_enc_td(BUF *dst, uint64_t type, uint64_t size, char *err)
{
  ...
  if (check)
    {
      strncpy(err, "Type/data parameter 'Type' value doesn't fit within"
	      " seven byte limit", PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  check = size >> 56;
  if (check)
    {
      strncpy(err, "Type/data parameter 'Size' value doesn't fit within"
	      " seven byte limit", PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
...
extern int data_dec_td(BUF *src, uint64_t *type, uint64_t *size, char *err)
{
  ...
  if (ret)
    strncpy(err, "Unexpected end of buffer while decoding type/data parameter",
	    PCCC_ERR_LEN);
  return ret;
}
...
static PCCC_RET_T enc_str(BUF *dest, const void *src, char *err)
{
  ...
  if (s->len > 82)
    {
      snprintf(err, PCCC_ERR_LEN, "String element with invalid length, %u. "
	       "82 maximum allowed value", s->len);
      return PCCC_EPARAM;
    }
  /*
