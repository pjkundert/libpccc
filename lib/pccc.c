...
static int get_addr(struct sockaddr_in *addr, const char *hostname,
		    in_port_t port);
			       ...
			       in_port_t link_port, const char *client_name)
{
  ...
  if (get_addr(&addr, link_host, link_port))
    {
      snprintf(con_priv->errstr, PCCC_ERR_LEN,
	       "Could not resolve hostname %s : %s", link_host,
	       hstrerror(h_errno));
      return PCCC_EPARAM;
    }
  ...
  if (con->fd < 0)
    {
      snprintf(con_priv->errstr, PCCC_ERR_LEN, "socket() failed : %s",
	       strerror(errno));
      return PCCC_EFATAL;
    }
 again:
  if (connect(con->fd, (struct sockaddr *)&addr, sizeof(struct sockaddr_in)))
    {
      ...
      snprintf(con_priv->errstr, PCCC_ERR_LEN, "Failed to connect : %s",
	       strerror(errno));
      return PCCC_ELINK;
    }
...
extern PCCC_RET_T pccc_read(PCCC *con)
{
  ...
  if (len < 0)
    snprintf(con_priv->errstr, PCCC_ERR_LEN, "Error reading : %s",
	     strerror(errno));
  else if (!len)
    strncpy(con_priv->errstr, "Remote end closed connection", PCCC_ERR_LEN);
  else return parse_link(con);
  return PCCC_ELINK;
}
...
extern PCCC_RET_T pccc_write(PCCC *con)
{
  ...
  if (!con_priv->connected)
    ...
    {
      snprintf(con_priv->errstr, PCCC_ERR_LEN, "Error writing : %s",
	       strerror(errno));
      return PCCC_ELINK;
    }
  return PCCC_SUCCESS;
...
extern PCCC_RET_T pccc_tick(PCCC *con)
{
  ...
  for (i = 0; i < con_priv->num_msgs; i++)
      ...
      if (msg->is_cmd && msg->expires)
	{
	  ...
	  if (now == -1)
	    {
	      snprintf(con_priv->errstr, PCCC_ERR_LEN, "time() failed : %s",
		       strerror(errno));
	      return PCCC_EFATAL;
	    }
	  /*
...
extern void pccc_errstr(PCCC *con, PCCC_RET_T err, char *buf, size_t len)
{
  ...
  switch (err)
    {
    case PCCC_SUCCESS:
      strncpy(buf, "Success", len);
      break;
    case PCCC_WREADY:
      strncpy(buf, "Success, data pending to be written to link layer", len);
      break;
    case PCCC_ENOCON:
      strncpy(buf, "Supplied connection pointer was invalid(NULL)", len);
      break;
    case PCCC_ECMD_NODELIVER:
      strncpy(buf, "Link layer service could not deliver command", len);
      break;
    case PCCC_ECMD_TIMEOUT:
      strncpy(buf, "Timed out awaiting a reply", len);
      break;
    case PCCC_ECMD_REPLY:
      strncpy(buf, "Reply contained an error", len);
      break;
    case PCCC_ECMD_NOBUF:
      strncpy(buf, "No message buffers available to process command", len);
      break;
    case PCCC_ELINK:
      strncpy(buf, "Link layer service connection error", len);
      break;
    case PCCC_EPARAM:
      strncpy(buf, "Invalid parameter specified", len);
      break;
    case PCCC_EFATAL:
      strncpy(buf, "Fatal error", len);
      break;
    case PCCC_EOVERFLOW:
      strncpy(buf, "Buffer overflow", len);
      break;
    default:
      strncpy(buf, "Unknown error", len);
      break;
    }
...
static PCCC_RET_T send_reg(PCCC *con, const char *name)
{
  ...
  if (name == NULL)
    {
      strncpy(con_priv->errstr, "Invalid pointer(NULL) to client name",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  len = strlen(name);
  if (!len)
    {
      strncpy(con_priv->errstr, "Client name cannot be emtpy", PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  if (len > PCCC_NAME_LEN)
    {
      snprintf(con_priv->errstr, PCCC_ERR_LEN,
	       "Client name too long, %u characters max", PCCC_NAME_LEN);
      return PCCC_EPARAM;
    }
  ...
  if (pccc_write(con) != PCCC_SUCCESS)
    {
      snprintf(con_priv->errstr, PCCC_ERR_LEN,
	       "Failed to send registration message : %s", strerror(errno));
      return PCCC_ELINK;
    }
  return PCCC_SUCCESS;
}
...
static void rcv_nak(PCCC *con)
{
  MSG *cur = ((PCCC_PRIV *)con->priv_data)->cur_msg;
  msg_flush(cur);
