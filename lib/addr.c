...
extern PCCC_RET_T addr_enc_plc(BUF *dst, const PCCC_PLC_ADDR *src, char *err)
{
  switch (src->type)
    ...
    case PCCC_PLC_ADDR_ASCII:
      ...
      break;
    default:
      strncpy(err, "Unknown PLC address type", PCCC_ERR_LEN);
      break;
    }
  return PCCC_EPARAM;
}
...
static PCCC_RET_T enc_plc_lba(BUF *dst, const PCCC_PLC_LBA *adr, char *err)
{
  ...
  if (adr->num_lvl > 7)
    {
      strncpy(err, "Number of address levels cannot be greater than seven",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;;
    }
  /*
  ...
  for (i = adr->num_lvl; i--;)
    {
      if ((adr->lvl[i] > 999))
	{
	  strncpy(err, "PLC logical binary address level values must be"
		  " less than 1000", PCCC_ERR_LEN);
	  return PCCC_EPARAM;
	}
      mask |= 1 << i;
    }
