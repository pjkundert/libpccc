...
typedef struct _pccc_priv
{
  ...
  size_t num_msgs;
  MSG *cur_msg; /* Pointer to current message being transmitted. */
  MSG *msgs; /* */
...
extern void msg_flush(MSG *m);
extern void msg_free(PCCC_PRIV *p);

extern PCCC_RET_T cmd_init(PCCC *con, MSG **pm, UFUNC notify, RFUNC reply,
			   uint8_t dnode, void *udata, uint8_t cmd,
			   uint8_t func);
extern PCCC_RET_T cmd_send(PCCC *con, MSG *cmd);

/*
 * Reply handlers. All reply handlers must conform to the same prototype.
 */
extern int reply_Echo(BUF *rply, MSG *cmd, char *err);
extern int reply_ProtectedTypedLogicalRead(BUF *rply, MSG *cmd, char *err);
extern int reply_ReadSLCFileInfo(BUF *rply, MSG *cmd, char *err);
extern int reply_ReadLinkParam(BUF *rply, MSG *cmd, char *err);
extern int reply_Dummy(BUF *rply, MSG *cmd, char *err);
...
extern PCCC_RET_T data_enc_array(MSG *msg, char *err);
extern PCCC_RET_T data_dec_array(BUF *rply, MSG *msg, char *err);
