...
static PCCC_RET_T ptl_init(PCCC *con, MSG **pm, UFUNC notify, uint8_t dnode,
...
*/
extern PCCC_RET_T pccc_cmd_BitWrite
(PCCC *con, UFUNC notify, uint8_t dnode, const PCCC_PLC_ADDR *addr,
 uint16_t set, uint16_t reset)
{
  MSG *cmd;
  PCCC_RET_T ret;
  PCCC_PRIV *con_priv;
  ...
  if (addr == NULL)
    {
      strncpy(con_priv->errstr, "Address pointer cannot be NULL",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  if (set & reset)
    {
      strncpy(con_priv->errstr, "Bits must be mutually exclusive in masks",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
...
*/
extern PCCC_RET_T pccc_cmd_ChangeModeMicroLogix1000
(PCCC *con, UFUNC notify, uint8_t dnode, PCCC_MODE_T mode)
{
  MSG *cmd;
  PCCC_RET_T ret;
  PCCC_PRIV *con_priv;
  ...
  switch (mode)
    ...
    case PCCC_MODE_RUN:
      ...
      break;
    default:
      strncpy(con_priv->errstr,
	      "Command does not support selected processor mode",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
      break;
    }
  ...
  if (buf_append_byte(cmd->buf, mode_val))
    {
      strncpy(con_priv->errstr, "pccc_cmd_ChangeModeMicroLogix1000()",
	      PCCC_ERR_LEN);
      msg_flush(cmd);
      return PCCC_EOVERFLOW;
    }
...
- PCCC_EPARAM if the given mode was invalid.
*/
extern PCCC_RET_T pccc_cmd_ChangeModeSLC500
(PCCC *con, UFUNC notify, uint8_t dnode, PCCC_MODE_T mode)
{
  MSG *cmd;
  PCCC_RET_T ret;
...
*/
extern PCCC_RET_T pccc_cmd_ProtectedTypedLogicalRead3AddressFields
(PCCC *con, UFUNC notify, uint8_t dnode, void *udata, PCCC_FT_T file_type,
 uint16_t file, uint16_t element, uint16_t sub_element, size_t num_elements)
{
  MSG *cmd;
  PCCC_RET_T ret;
  if (con == NULL) return PCCC_ENOCON;
  ret = ptl_init(con, &cmd, notify, dnode, udata, 0xa2, file_type, file,
...
PCCC_EPARAM - If one of the parameters was invalid.
*/
extern PCCC_RET_T pccc_cmd_ProtectedTypedLogicalRead2AddressFields
(PCCC *con, UFUNC notify, uint8_t dnode, void *udata, PCCC_FT_T file_type,
 uint16_t file, uint16_t element, size_t num_elements)
{
  MSG *cmd;
  PCCC_RET_T ret;
  if (con == NULL) return PCCC_ENOCON;
  ret = ptl_init(con, &cmd, notify, dnode, udata, 0xa1, file_type, file,
		 element, 0, num_elements);
  if (ret != PCCC_SUCCESS) return ret;
  return cmd_send(con, cmd);
}
...
PCCC_EPARAM - If one of the parameters was invalid.
*/
extern PCCC_RET_T pccc_cmd_ProtectedTypedLogicalWrite3AddressFields
(PCCC *con, UFUNC notify, uint8_t dnode, void *udata, PCCC_FT_T file_type,
 uint16_t file, uint16_t element, uint16_t sub_element, size_t num_elements)
{
  MSG *cmd;
  PCCC_RET_T ret;
  if (con == NULL) return PCCC_ENOCON;
  ret = ptl_init(con, &cmd, notify, dnode, udata, 0xaa, file_type, file,
		 element, sub_element, num_elements);
  if (ret != PCCC_SUCCESS) return ret;
...
PCCC_EPARAM - If one of the parameters was invalid.
*/
extern PCCC_RET_T pccc_cmd_ProtectedTypedLogicalWrite2AddressFields
(PCCC *con, UFUNC notify, uint8_t dnode, void *udata, PCCC_FT_T file_type,
 uint16_t file, uint16_t element, size_t num_elements)
{
  MSG *cmd;
  PCCC_RET_T ret;
  if (con == NULL) return PCCC_ENOCON;
  ret = ptl_init(con, &cmd, notify, dnode, udata, 0xa9, file_type, file,
		 element, 0, num_elements);
  if (ret != PCCC_SUCCESS) return ret;
  ret = data_enc_array(cmd, ((PCCC_PRIV *)con->priv_data)->errstr);
  if (ret != PCCC_SUCCESS)
...
- PCCC_EPARAM if the given mode was invalid.
*/
extern PCCC_RET_T pccc_cmd_SetCPUMode
(PCCC *con, UFUNC notify, uint8_t dnode, PCCC_MODE_T mode)
{
  MSG *cmd;
  PCCC_RET_T ret;
...
*/
extern PCCC_RET_T pccc_cmd_ReadSLCFileInfo
(PCCC *con, UFUNC notify, uint8_t dnode, PCCC_SLC_FI_T *udata,
 uint8_t file_num)
{
  MSG *cmd;
  PCCC_RET_T ret;
  ...
  ret = cmd_init(con, &cmd, notify, reply_ReadSLCFileInfo, dnode,
		 (void *)udata, 0x0f, 0x94);
  if (ret != PCCC_SUCCESS) return ret;
 /*
...
- SLC 5/05, Series B, Proc. Revision 4. OS Cat Num 501, Series C, FRN 6
*/
extern PCCC_RET_T pccc_cmd_DisableForces
(PCCC *con, UFUNC notify, uint8_t dnode)
{
  MSG *cmd;
  PCCC_RET_T ret;
  if (con == NULL) return PCCC_ENOCON;
...
PCCC_EPARAM If one of the parameters was invalid.
*/
extern PCCC_RET_T pccc_cmd_ProtectedTypedLogicalWriteWithMask
(PCCC *con, UFUNC notify, uint8_t dnode, void *udata, PCCC_BIN_T mask,
 PCCC_FT_T file_type, uint16_t file, uint16_t element, uint16_t sub_element,
 size_t num_elements)
{
  MSG *cmd;
  PCCC_RET_T ret;
  ...
  if (buf_append_word(cmd->buf, htols(mask)))
    {
      strncpy(con_priv->errstr,
	      "pccc_cmd_ProtectedTypedLogicalWriteWithMask()", PCCC_ERR_LEN);
      msg_flush(cmd);
      return PCCC_EOVERFLOW;
    }
  ret = data_enc_array(cmd, ((PCCC_PRIV *)con->priv_data)->errstr);
...
PCCC_EPARAM if one of the parameters was invalid.
*/
extern PCCC_RET_T pccc_cmd_ReadModifyWrite
(PCCC *con, UFUNC notify, uint8_t dnode, const PCCC_PLC_ADDR *addr,
 const uint16_t *and, const uint16_t *or, size_t sets)
{
  MSG *cmd;
  PCCC_RET_T ret;
  PCCC_PRIV *con_priv;
  ...
  if (and == NULL)
    {
      strncpy(con_priv->errstr, "AND mask pointer cannot be NULL",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  if (or == NULL)
    {
      strncpy(con_priv->errstr, "OR mask pointer cannot be NULL",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  if (!sets)
    {
      strncpy(con_priv->errstr, "Number of sets must be non-zero",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  ...
  for (i = 0; i < sets; i++)
	...
	{
	  strncpy(con_priv->errstr, "pccc_cmd_ReadModifyWrite()",
		  PCCC_ERR_LEN);
	  msg_flush(cmd);
	  return PCCC_EOVERFLOW;
	}
	...
	{
	  strncpy(con_priv->errstr,
		  "Number of sets exceeded maximum command size",
		  PCCC_ERR_LEN);
	  msg_flush(cmd);
	  return PCCC_EPARAM;
	}
			   ...
			   uint16_t sub_element, size_t num_elements)
{
  ...
  if (sub_element)
    {
      strncpy(con_priv->errstr, "Nonzero subelement values not supported",
	      PCCC_ERR_LEN);
      return PCCC_EPARAM;
    }
  /*
  ...
  if (bytes > 236)
    {
      ...
      snprintf(con_priv->errstr, PCCC_ERR_LEN,
	       "Too many elements. Data type allows %u elements max",
	       max_elements);
      return PCCC_EPARAM;
    }
